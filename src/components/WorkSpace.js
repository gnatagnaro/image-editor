...
function WorkSpace({ tool, form, handleFormChange }) {
  ...
  const mouseMove = (e) => {
    ...
    if (mouseState && tool === "hand" && form === "none") {
      ...
      canvas.style.left =
        Number(canvas.style.left.slice(0, -2)) + offsetX + "px";
    }
  };
  ...
  const mouseScroll = (e) => {
    if (control) {
      ...
      canvas.style.height =
        Number(canvas.style.height.slice(0, -2) * (1 + (e.deltaY * -1) / 800)) +
        "px";
      canvas.style.width =
        Number(canvas.style.width.slice(0, -2) * (1 + (e.deltaY * -1) / 800)) +
        "px";
    } else {
      canvas.style.top =
        Number(canvas.style.top.slice(0, -2)) + e.deltaY * -1 + "px";
      canvas.style.left =
        Number(canvas.style.left.slice(0, -2)) + e.deltaX * -1 + "px";
    }
  };
  ...
  const fileChange = async (e) => {
    ...
    setImage(img);
    setUrl(null);
    setImageWidth(img.width); // Установка ширины изображения
    setImageHeight(img.height); // Установка высоты изображения
  };
  ...
  const getCoordinate = (e) => {
    ...
    if (mouseState && tool === "brush") {
      ...
      console.log(
        `${lastCoords[0][lastCoords.length - 1]}, ${
          lastCoords[1][lastCoords.length - 1]
        }`
      );

      context.beginPath();
      context.moveTo(
        lastCoords[0][lastCoords.length - 2],
        lastCoords[1][lastCoords.length - 2]
      );
  ...
  return (
    <div className="workspace">
      ...
      <div className="workspace__coordinates">
        <p className="workspace__coordinate">X: {coordinates[0]}</p>
        <p className="workspace__coordinate">Y: {coordinates[1]}</p>
        <div
          className="workspace__pixel-color"
          style={{ background: `rgb(${pixelColor})` }}
        ></div>
        <p className="workspace__coordinate">
          Ширина изображения: {imageWidth}px
        </p>{" "}
        {/* Отображение ширины */}
        <p className="workspace__coordinate">
          Высота изображения: {imageHeight}px
        </p>{" "}
        {/* Отображение высоты */}
        <p className="workspace__coordinate">RGB: {pixelColor}</p>{" "}
        {/* Отображение значения RGB */}
      </div>
    </div>
  );
}
